...
class ConsistentHashMap:
    def __init__(self, num_servers, num_slots):
        ...
        for i in range(
            1, num_servers + 1
        ):  # Start from 1 to match Docker service names
            self.add_server(str(i))  # Ensure server IDs are strings

    def hash_request(self, request_id):
        hashed_value = (
            int(hashlib.md5(str(request_id).encode()).hexdigest(), 16) % self.num_slots
        )
    ...
    def hash_server(self, server_id, virtual_id):
        hashed_value = (
            int(hashlib.md5(f"{server_id}-{virtual_id}".encode()).hexdigest(), 16)
            % self.num_slots
        )
        print(
            f"Hashed value for server {server_id} with virtual id {virtual_id}: {hashed_value}"
        )
        return hashed_value
    ...
    def add_server(self, server_id):
        ...
        for virtual_id in range(self.num_virtual_servers):
            ...
            while slot in self.virtual_servers:
                ...
                slot = (
                    slot + (2 * virtual_id) + 1
                ) % self.num_slots  # Quadratic probing
                if probing_attempts > self.num_slots:
                    print(
                        f"Error: Too many probing attempts for server {server_id} virtual_id {virtual_id}. Exiting."
                    )
                    return
            self.virtual_servers[slot] = (server_id, virtual_id)
            self.servers.append(slot)
            print(
                f"Assigned virtual server {virtual_id} of server {server_id} to slot {slot}"
            )
        self.servers.sort()
